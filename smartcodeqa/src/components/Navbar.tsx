						...
						{navLinks.map((link, index) => (
							<div key={index} className="relative">
								{link.dropdown ? (
									...
									>
										<motion.button
											className="flex items-center gap-1 px-4 py-2 text-text-secondary hover:text-text-primary transition-colors cursor-hover"
									...
									<motion.div whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }}>
										<Link
											href={link.href!}
											className="relative px-4 py-2 text-text-secondary hover:text-text-primary transition-colors cursor-hover group"
										>
											{link.label}
											<motion.span
												className="absolute bottom-0 left-4 right-4 h-0.5 bg-gradient-to-r from-accent-blue to-accent-cyan rounded-full"
												initial={{ scaleX: 0 }}
												whileHover={{ scaleX: 1 }}
